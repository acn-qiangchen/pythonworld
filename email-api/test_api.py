...
def test_send_email(to_email, from_email):
    ...
    payload = {
        "to_email": to_email,
        "subject": "Test Email from Email API",
        "body": "This is a test email sent via the Email API service using AWS SES.",
    ...
    response = requests.post(f"{BASE_URL}/api/v1/send-email", json=payload)
    print(f"Status Code: {response.status_code}")
    print(f"Response: {json.dumps(response.json(), indent=2)}")
...
def main():
    ...
    try:
        ...
        print("  OR")
        print("  docker run -d -p 8000:8000 --env-file .env email-api:latest")
